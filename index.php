<?php
    ...
    function getData4KW($openDbConnection, $year, $kw, $kwList) {
        ...
        // Query: Top 40 Einträge der gewählten KW direkt aus top40
        $query = "
            SELECT 
                t.Platz AS platz,
                t.Titel AS titel,
        ...
        while ($row = $result->fetch_assoc()) {
            if ($currentRank != $row["platz"]) {
                $entry = new Top40Entry(
                    ...
                    $kw,
                    $year,
                    null  // song_id gibt es hier nicht
                );

                // Vorwochen-Rang ermitteln
...
<html lang="de">
    ...
    <!-- Week selection form -->
        ...
        <?php endif; ?> <!--raus, in requires auslagern-->

        <?php if ($showWarning): ?>
            <div class="warning">Keine Daten der Vorwoche vorhanden!</div>
        <?php else: ?> <!--raus, in requires auslagern-->
            <table>
                <tr>
